...
根據 `DEVELOPMENT_FLOW.md` 規範，本專案採用多 AI 工具輪流協作模式：

1. **架構防禦**: 嚴禁未經授權擅自重構，保持風格一致性
2. **Git 分支策略**: 
   - 新功能: `feat/功能名稱` (從 main 分支切出)
   - Bug 修復: `fix/問題描述` (從 main 分支切出)
3. **原子化提交**: 每完成獨立邏輯修改必須立即 commit
4. **AI 輪班交接**: 
   - 交接檔案: `AI_HANDOFF.md`
   - 交接觸發關鍵字: 下班了、交班、任務結束等
   - 自動更新交接報告並顯示標準提示

此架構設計使得專案能夠在多個 AI 代理程式間穩暢交接，同時保持程式碼品質和系統完整性。