...
1. **Git 分支**：確認目前不在 `main` 或 `develop` 分支。若在受保護分支上，先提醒使用者切出 `feat/功能名稱` 或 `fix/問題描述` 分支。**例外**：僅更新 `AI_HANDOFF.md`、`CLAUDE.md`、`README.md`、`CHANGELOG.md` 等文件時，可直接 commit 到 develop，不需要開分支。
